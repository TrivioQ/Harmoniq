        ...
        {environments.map((env) => (
          <div key={env.id} className="rounded-xl border bg-card text-card-foreground shadow-sm p-6 flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{env.slug} &middot; {t('hostApp', { hostAppName: env.hostAppName })}</p>
            </div>
            <div>
              {env.isDefault && <span className="bg-primary text-primary-foreground text-xs px-2 py-1 rounded">{t('default')}</span>}
            </div>
          </div>
        ))}
